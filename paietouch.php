...
//  $codeServ = "PAIEMENTMARCHAND_MTN_CI";
  ...
  $codeServ = "BF_PAIEMENTMARCHAND_OM_TP";
	$agent = "BFVAS1933";
	$login = "22964008585";
	$gui = 0;
	$user = "EFB77C5C17844580828661841244B2F7A980BA648C4944952B25C68E18CC948F";
...
if (isset($opt[0]) AND $opt[0]=='AFPRCH') {
  ...
  $amount = $elt['montant_transaction'];
	...
	if (end($lib) == "don") {
		$amount = ($_GET['s'] == undefined) ? $elt['montant_transaction'] : $_GET['s'];
		//echo $amount;
	} else {
    ...
    // var_dump($datee);
     ...
     // echo $key;
		...
		if ($key !=="") {
				 ...
				 if ($err) {
					  ...
					  echo $first_response2;
						...
						if (isset($first_respons2->status) AND $first_respons2->status == 201) {
							$req_ajt = "INSERT INTO transactionOM (status_transactionOM, message_transactionOM, libelle_transactionOM, payToken_transactionOM, notifToken_transactionOM, id_transaction) VALUES (:status_transactionOM, :message_transactionOM, :libelle_transactionOM, :payToken_transactionOM, :notifToken_transactionOM, :id_transaction)";
				        $req_aj_tok = $bdd -> prepare($req_ajt);
		 ...
		 //echo "https://api.gutouch.com/dist/api/touchpayapi/v1/".$agent."/transaction?loginAgent=".$login."&passwordAgent=".$pwdagnt;
