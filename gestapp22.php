...
/* ------------- FONCTION SUPPRIMER -----------------*/
        ...
        $re = "SELECT * FROM album INNER JOIN artiste
  			...
  			$alb = $al->fetch();
    			...
    			//var_dump($alb);
  				...
  				$affich_el["s"][$i]['name'] = utf8_decode($alb['titre_album'])." (l'album entier)";
  				$affich_el["s"][$i]['artist'] = utf8_decode($alb['nom_artiste']);
  				$affich_el["s"][$i]['cover_art_url'] = utf8_decode($alb['cover_artiste']);
  				$affich_el["s"][$i]['cover_url'] = utf8_decode($alb['cover_album']);
  				$affich_el["s"][$i]['url'] = utf8_decode($alb['url_album']);
  				$affich_el["s"][$i]['f'] = utf8_decode($alb['fichier_album']);
  ...
  function checkStat(){
    ...
    //var_dump($_POST);
		$affich_el[] = '';
    $re = "SELECT * FROM ".$_POST['table']." WHERE reference_transaction = '".$_POST['ref']."'";
		$un = $bdd -> query($re);
		//$nbuser = $un->rowCount();
		$elt = $un->fetch();
    //var_dump($elt);
...
switch ($_GET["nom_fct"]){
    ...
    // error_log()
		...
		case 22 : //cas liste son togolais
		 listPPOPU();
		break;

		case 23 : //cas liste son togolais
		 rechApi();
		break;
