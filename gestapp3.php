...
/* ------------- FONCTION SUPPRIMER -----------------*/
    ...
    $record = geoip_record_by_name($_POST['ip']);
    var_dump($record);
		...
		if($_GET['table'] == 'son'){
			...
			$re = "SELECT * FROM son LEFT JOIN album
										  ...
										  LEFT JOIN lyric
										  ON lyric.id_son = son.id_son ORDER BY son.titre_son";
			$son = $bdd -> query($re);
			$nbson = $son -> rowCount();
			//echo $re;
			$affich_el[0]=1;
	...
	function listAASfiltre(){
      ...
      $nbson3 = $son3 -> rowCount();
        while ($sons3 = $son3->fetch()){
          //var_dump ($sons);


          $re1 = "SELECT * FROM telechargements WHERE id_son =".$sons3[0];
          $nb = $bdd -> query($re1);
          $nbs = $nb->rowCount();
          //$j = 0;
          $affich_el['sons'][$i]['id'] = $sons3['id_son'];
          $affich_el['sons'][$i]['name'] = utf8_decode($sons3['titre_son']);
          $affich_el['sons'][$i]['artist'] = utf8_decode($sons3['nom_artiste']);
          $affich_el['sons'][$i]['album'] = utf8_decode($sons3['titre_album']);
          $affich_el['sons'][$i]['url'] = utf8_decode($sons3['url_son']);
          $affich_el['sons'][$i]['cover_art_url'] = utf8_decode($sons3['cover_artiste']);
          $affich_el['sons'][$i]['cover_url'] = utf8_decode($sons3['cover_son']);
          $affich_el['sons'][$i]['price'] = $sons3['prix_son'];
          $affich_el['sons'][$i]['downloads'] = $nbs;
          $affich_el['sons'][$i]['lyrics'] = utf8_decode($sons3['texte_lyric']);
          $affich_el['sons'][$i]['reference'] = $sons3['reference_son'];
          $affich_el['sons'][$i]['lien'] = $sons3['lien_son'];
            ...
            if(isset($_GET['art'])){
              ...
              $reA = "SELECT * FROM album INNER JOIN artiste
        			...
        			while ($alb = $al->fetch() ){
        				...
        				//$j = 0;
          			//var_dump($alb);
        				$affich_el['albums'][$i]['id'] = $alb['id_album'];
        				...
        				$affich_el['albums'][$i]['price'] = $alb['prix_album'];
        				$affich_el['albums'][$i]['downloads'] = $nba;
        ...
        while ($sons = $son->fetch() ){
  				...
  				$nbd = $nb->rowCount();
  				//$j = 0;
  				$affich_el['asons'][$i]['id'] = $sons['id_son'];
  				...
  				$affich_el['asons'][$i]['price'] = $sons['prix_son'];
        ...
        $nbson = $son -> rowCount();
          while ($sons = $son->fetch() ){
            ...
            $nbs = $nb->rowCount();
            //$j = 0;
            $affich_el['sons'][$i]['id'] = $sons['id_son'];
            $affich_el['sons'][$i]['name'] = utf8_decode($sons['titre_son']);
            $affich_el['sons'][$i]['artist'] = utf8_decode($sons['nom_artiste']);
            $affich_el['sons'][$i]['album'] = utf8_decode($sons['titre_album']);
            $affich_el['sons'][$i]['url'] = utf8_decode($sons['url_son']);
            $affich_el['sons'][$i]['cover_art_url'] = utf8_decode($sons['cover_artiste']);
            $affich_el['sons'][$i]['cover_url'] = utf8_decode($sons['cover_son']);
            $affich_el['sons'][$i]['price'] = $sons['prix_son'];
            $affich_el['sons'][$i]['downloads'] = $nbs;
            $affich_el['sons'][$i]['lyrics'] = utf8_decode($sons['texte_lyric']);
            $affich_el['sons'][$i]['reference'] = $sons['reference_son'];
            $affich_el['sons'][$i]['lien'] = $sons['lien_son'];
          ...
          if(isset($_GET['art'])){
            ...
            while ($alb = $al->fetch() ){
              ...
              $affich_el['albums'][$i]['artist'] = utf8_decode($alb['nom_artiste']);
              $affich_el['albums'][$i]['cover_art_url'] = utf8_decode($alb['cover_artiste']);
              $affich_el['albums'][$i]['cover_url'] = $alb['cover_album'];
              $affich_el['albums'][$i]['url'] = utf8_decode($alb['url_album']);
              $affich_el['albums'][$i]['price'] = $alb['prix_album'];
              $affich_el['albums'][$i]['downloads'] = $nba;
              $affich_el['albums'][$i]['reference'] = $alb['reference_album'];
			...
			while ($sons = $son->fetch() ){
				...
				$affich_el['sons'][$i]['url'] = utf8_decode($sons['url_son']);
				$affich_el['sons'][$i]['cover_art_url'] = utf8_decode($sons['cover_artiste']);
				$affich_el['sons'][$i]['cover_url'] = $sons['cover_son'];
				$affich_el['sons'][$i]['price'] = $sons['prix_son'];
				$affich_el['sons'][$i]['downloads'] = $nbs;
				$affich_el['sons'][$i]['lyrics'] = utf8_decode($sons['texte_lyric']);
				$affich_el['sons'][$i]['reference'] = $sons['reference_son'];
  ...
  function checkStat(){
	...
	function inscr(){
		require 'connexion.php';
		$req = $bdd->query("SHOW TABLE STATUS FROM evallesse LIKE 'user' ");
		$donnees = $req->fetch();
		...
		$req_ajd = "INSERT INTO user (telephone_user) VALUES (:telephone_user)";
			$req_aj_don = $bdd -> prepare($req_ajd);
