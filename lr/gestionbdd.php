 ...
 function genererChaineAleatoire()
{
 ...
 return $chaine;
	...
	function ajouter(){
	 ...
	 if($tabl == 'maison' OR $tabl == "artiste" OR $tabl == "auteurS"){
	 ...
	 if (preg_match("#fichier#", $table[$i])) {
	 	$valeur[$i] = basename($_FILES[$ch_fich[$indxfich]]['name']);
	 }
			...
			for ($o=0; $o < $indxfich; $o++) {
	 		 ...
	 		 $cover = (preg_match("#cover#", $ch_fich[$o])) ? 'cover-' : '';
			 	if($cover !== ''){
					...
					if($tabl == "son"){
						move_uploaded_file($_FILES[$ch_fich[$o]]['tmp_name'], '../../file/' .basename($_FILES[$ch_fich[$o]]['name']));
						$target_path = '../../file/' .basename($_FILES[$ch_fich[$o]]['name']);
						$target_path_2 = '../site/file/' .basename($_FILES[$ch_fich[$o]]['name']);
	 ...
	 $table[$i] = $champ["Field"]; // recuperer le nom des colums de la table
		...
		if ($e == 11) {
			$req_aff.=' INNER JOIN client_typeClient
				...
				case '5' :
				$tabindic = array(1,2,5,6,7,12,16,3);
				break;
				...
				case '7' :
				$tabindic = array(5,4,3,1,2,6,7,23);
				break;
		...
		while($affich = $aff->fetch()){
			...
			if($j == $ttab){
				...
				if($e == 11){
					...
					$affich_el[$i][$j] = $affich[7];
					$j++;*/
					$affich_el[$i][$j] = "<a type='button' href='modifierusr.php?indx=".$affich[0]."&t=userplaylist' class='btn btn-block btn-default' style=' '>Modifier</a>";
					$j++;
					$affich_el[$i][$j] = "<div id='".$affich[0]."' ><button type='button' data-id='".$affich[0]."' class='btn btn-block btn-info' style=' '>Supprimer</button></div><div id='".$affich[0]."r2' class='hide'><button type='button' data-id='".$affich[0]."' class='btn btn-block btn-danger valid_sup'>Valider</button></div>";
...
function majun(){
	...
	if($tabl == "envoi"){
			...
			$req3='SELECT * FROM agent
						INNER JOIN pointcash
						ON agent.id_pointcash = pointcash.id_pointcash
						INNER JOIN admpays
						on pointcash.id_admpays = admpays.id_admpays
						WHERE agent.id_agent = '.$amod['id_agent'];
			$ap1  = $bdd ->query($req3);
			$admp1 = $ap1 -> fetch();
			$list_mail[0] = $admp1['email_admpays'];
			$list_nom[0] = $admp1['pseudo_admpays'];

			$req2='SELECT * FROM agent
							INNER JOIN pointcash
							ON agent.id_pointcash = pointcash.id_pointcash
							INNER JOIN admpays
							on pointcash.id_admpays = admpays.id_admpays
							WHERE pointcash.id_pointcash = '.$amod['id_pointcash'];
			$ap2  = $bdd ->query($req2);
			$admp2 = $ap2 -> fetch();

			if ($_POST['devise_envoi'] == $admp2['devise_admpays']){
				$monteaj1 = $_POST['montant_envoi'] - $amod['montant_envoi'];
			}else {
				$reqtx="SELECT * FROM taux WHERE devisea_taux = '".$_POST['devise_envoi']."' AND deviseb_taux = '".$admp2['devise_admpays']."'";
				$tx  = $bdd ->query($reqtx);
				$taux = $tx -> fetch();
				$monteaj1 = ($_POST['montant_envoi'] - $amod['montant_envoi'])*$taux['taux_taux'];
			}
			if ($_POST['devise_envoi'] == $admp1['devise_admpays']){
				$monteaj2 = ($_POST['montant_envoi'] - $amod['montant_envoi'])+($_POST['commi_envoi'] - $amod['commi_envoi']);
			}else {
				$reqtx="SELECT * FROM taux WHERE devisea_taux = '".$_POST['devise_envoi']."' AND deviseb_taux = '".$admp1['devise_admpays']."'";
				$tx  = $bdd ->query($reqtx);
				$taux = $tx -> fetch();
				$monteaj2 = (($_POST['montant_envoi'] - $amod['montant_envoi'])+($_POST['commi_envoi'] - $amod['commi_envoi']))*$taux['taux_taux'];
			}

			$nfond1 = $admp1['fond_pointcash'] - $monteaj2;
			$nfond2 = $admp2['fond_pointcash'] + $monteaj1;

			echo $admp1['id_pointcash'].' -- '.$admp2['id_pointcash'];
			echo '<br>'.$nfond2.' -- '.$nfond1;

			$reqfond1 = "UPDATE pointcash SET fond_pointcash = :fond_pointcash WHERE id_pointcash = :id_pointcash";
			$reqfonds1 = $bdd -> prepare($reqfond1);
			$reqfonds1 ->bindParam(":fond_pointcash", $nfond1);
			$reqfonds1 ->bindParam("id_pointcash", $admp1['id_pointcash']);
			$reqfonds1 -> execute();

			$reqfond = "UPDATE pointcash SET fond_pointcash = :fond_pointcash WHERE id_pointcash = :id_pointcash";
			$reqfonds = $bdd -> prepare($reqfond);
			$reqfonds ->bindParam(":fond_pointcash", $nfond2);
			$reqfonds ->bindParam("id_pointcash", $admp2['id_pointcash']);
			$reqfonds -> execute();


		} else if($tabl == "reception"){
			...
			if ($_POST['devise_reception'] == $admp1['devise_admpays']){
				$monteaj1 = $_POST['montant_reception'] - $amod['montant_reception'];
			}else {
				$reqtx="SELECT * FROM taux WHERE devisea_taux = '".$_POST['devise_envoi']."' AND deviseb_taux = '".$admp1['devise_admpays']."'";
				$tx  = $bdd ->query($reqtx);
				$taux = $tx -> fetch();
				$monteaj1 = ($_POST['montant_reception'] - $amod['montant_reception'])*$taux['taux_taux'];
			}
			$nfond1 = $admp1['fond_pointcash'] - $monteaj1;
