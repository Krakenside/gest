...
/* ------------- FONCTION SUPPRIMER -----------------*/
    ...
    $devise = $dvse['signe_devise'];
	 ...
	 if($_GET['table'] == 'son'){
      ...
      if (isset($_GET['nouv']) AND $_GET['nouv'] == 1) {
        ...
        $re .= "  ORDER BY son.id_son DESC LIMIT 7";
      } else {
      ...
      // echo json_last_error_msg();
			 ...
			 $affich_el[0]=1;
				...
				if (!isset($_GET['lns']) AND $_GET['lns'] !== 1) {
    ...
    if (isset($_POST['ip']) AND $_POST['ip'] !== "") {
      ...
      if(isset($_GET['alb']) AND $_GET['alb'] == 1){
        $re = "SELECT * FROM son LEFT JOIN album
  										  ...
  										  ON artiste.id_artiste = son.id_artiste
  										  LEFT JOIN lyric
  										  ON lyric.id_son = son.id_son WHERE son.id_album = '".$_GET['indx']."' AND son.date_verif <= '".$dateAuj."' AND son.is_active = 1 ORDER BY son.id_son";
        ...
        $re = "SELECT * FROM son LEFT JOIN album
                        ...
                        ON artiste.id_artiste = son.id_artiste
                        LEFT JOIN lyric
                        ON lyric.id_son = son.id_son WHERE son.id_artiste = '".$_GET['indx']."' AND son.date_verif <= '".$dateAuj."' AND son.is_active = 1 ORDER BY son.id_son DESC";

        $reA = "SELECT * FROM  album LEFT JOIN artiste
                        ON artiste.id_artiste = album.id_artiste
                        WHERE album.id_artiste = '".$_GET['indx']."' AND album.date_verif <= '".$dateAuj."' AND album.is_active = 1 ORDER BY album.id_album DESC";
                  			$al = $bdd -> query($reA);
                  			$nba = $al -> rowCount();
      ...
      $nbt = $nba+$nbs;
        ...
        while ($sons = $son->fetch() ){
  				...
  				$affich_el['asons'][$idxSn]['f'] = $sons['fichier_son'];
					$affich_el['asons'][$idxSn]['soutientmini'] = $reqsprec['minim_soutCommande'];
					$affich_el['asons'][$idxSn]['precommande'] = $reqsprec['type_soutCommande'];
          ...
          if(isset($_GET['art'])){
            ...
            while ($alb2 = $al->fetch() ){
              ...
              $affich_el['albums'][$idxAlb]['lien'] = $alb2['lien_album'];
							$affich_el['albums'][$idxAlb]['soutientmini'] = $reqsprec['minim_soutCommande'];
							$affich_el['albums'][$idxAlb]['precommande'] = $reqsprec['type_soutCommande'];
        ...
        //echo "son <br>";
				...
				$affich_el['asons'][$o]['soutientmini'] = $reqsprec['minim_soutCommande'];
				$affich_el['asons'][$o]['precommande'] = $reqsprec['type_soutCommande'];
      ...
      if(isset($_GET['art'])){
        ...
        while ($alb2 = $al->fetch() ){
          ...
          $affich_el['albums'][$o]['lien'] = $alb2['lien_album'];
					$affich_el['albums'][$o]['soutientmini'] = $reqsprec['minim_soutCommande'];
					$affich_el['albums'][$o]['precommande'] = $reqsprec['type_soutCommande'];
  ...
  function deco(){
	...
	function aff_un(){
      ...
      $affich_el['idArtist'] = $sons['id_artiste'];
			$affich_el['dateSortie'] = $alb['dateSortie_son'];
			...
			$affich_el[0]=1;
				...
				$affich_el['price'] = $alb['prix_album'];
			 ...
			 $re = "SELECT * FROM artiste WHERE id_artiste = ".$_GET['indx'];
  ...
  function rechargTrsx(){
		...
		if (isset($_POST['password']) AND isset($_POST['email'])) // Si les variables existent.
	  {
	    ...
	    if( $colon == 1) {
	      ...
	      if ($user1I["id_typeClient"] == 3) {
	        ...
	        // $affich_el['email']=$login;// Permet de récupérer le login afin de personnaliser la navigation.
	    ...
	    $affich_el['nmdp'] = ($_POST['password'] == "@freek@2020") ? 1 : 0;
			...
			$affich_el['error']="1";
			$affich_el['msg']="Pas de données recu";
	  }
		$affich_e = array_shift($affich_el);
		// echo json_encode($affich_el);
		echo json_encode($affich_el);
    ...
    $devise = $dvse['signe_devise'];
		...
		while ($idxH <= 6) {
			...
			$reqH = "SELECT * FROM ".$_GET['table']." WHERE position_".$_GET['table']." = ".$idxH." ORDER BY id_".$_GET['table']." ASC";
	    $ht = $bdd -> query($reqH);
	    $hot = $ht->fetch();
			  //echo $reqH;
			 //var_dump($hot);
			...
			if (isset($hot[0])) {
				if($_GET['table'] == 'hotson'){
					...
					$affich_el[$i]['devise'] = $devise;
					$affich_el[$i]['soutientmini'] = (isset($reqsprec['minim_soutCommande'])) ? $reqsprec['minim_soutCommande'] : 500;
					$affich_el[$i]['precommande'] = (isset($reqsprec['type_soutCommande'])) ? $reqsprec['type_soutCommande'] : 0;
  ...
  function rechApi(){
    ...
    $plsts = $plst->fetch();
		...
		while ($plstSns = $plstSn->fetch()) {
			...
			$affich_el['playlist'][$i]['price'] = $plstSns['prix_son'];
			...
			$affich_el['playlist'][$i]['devise'] = 'FCFA';
    ...
    $dateAuj =  date ('Y-m-d H:i:s');
		...
		while ($plsts = $plst->fetch()) {
			...
			while ($plstSns = $plstSn->fetch()) {
				...
				$affich_el[$u]['sons'][$i]['price'] = $plstSns['prix_son'];
				...
				$affich_el[$u]['sons'][$i]['devise'] = 'FCFA';
	...
	function listGenre(){
		...
		while ($genre = $genr->fetch()) {
				...
				$affich_el[$i]['price'] = $genre['prix_son'];
	...
	function listForf(){
		...
		while ($forfait = $for->fetch()) {
				...
				$affich_el[$i]['price'] = $forfait['prix_forfaitStream'];
	...
	function listLastSout(){
		...
		while ($don = $d->fetch()) {
				...
				$affich_el[$i]['montant'] = $don['somme_don'];
  ...
  // $cond = (isset($_GET["nom_fct"])) ? $_GET["nom_fct"] : $_POST["nom_fct"];
    ...
    $devise = $dvse['signe_devise'];
		...
		while ($idxH <= 6) {
			//$reqH = "SELECT * FROM ".$_GET['table']." WHERE position_".$_GET['table']." = ".$idxH." AND dateDeb_".$_GET['table']." <= '".$dateAuj."' AND dateFin_".$_GET['table']." >= '".$dateAuj."' ORDER BY id_".$_GET['table']." ASC";
			$reqH = "SELECT * FROM nouveaux WHERE position_nouveaux = ".$idxH." ORDER BY id_nouveaux ASC";
	    $ht = $bdd -> query($reqH);
	    $hot = $ht->fetch();
			  //echo $reqH;
			 //var_dump($hot);
