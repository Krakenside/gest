...
$req ="SELECT DISTINCT telephone_transaction FROM transaction WHERE libelle_transaction='".$_GET['tx']."' AND statut_transaction !='SUCCESS'";

          $tr  = $bdd ->query($req);
          $nb_tranNok = $tr -> rowCount();
          //$nb_tranNok = (is_integer($nb_tranNok)) ? $nb_tranNok : 0;
          //var_dump($req);

$req1 ="SELECT DISTINCT reference_transaction FROM transaction WHERE libelle_transaction='".$_GET['tx']."' AND statut_transaction ='SUCCESS'";

          $trOk  = $bdd ->query($req1);
          $nb_tranOk = $trOk -> rowCount();
          //$nb_tranOk = (is_integer($nb_tranOk)) ? $nb_tranOk : 0;
$nb_tran = $nb_tranNok + $nb_tranOk;
$pourc_tran = ($nb_tran>0) ? round(($nb_tranOk * 100)/$nb_tran, 2) : 0;
$sDon=0;
if (isset($_GET['asa'])) {
  $reqd ="SELECT * FROM don INNER JOIN transaction
                            ON don.id_transaction = transaction.id_transaction
                            INNER JOIN artiste
                            ON artiste.id_artiste = don.id_artiste
                            WHERE statut_transaction='SUCCESS' AND artiste.id_artiste = ".$_GET['asa'];

            $don  = $bdd ->query($reqd);
            //echo $reqd;
            while($dons = $don->fetch()){
            if ($dons['id_pays'] !== "4" AND $dons['id_pays'] !== "6") {
              $sDon += $dons['somme_don'];
            } else {
              $reqTx='SELECT * FROM pays INNER JOIN taux
                                            ON pays.id_devise = taux.from_taux
                                            where pays.id_pays = '.$dons['id_pays'].' AND to_taux = 1';
        				$rqTx  = $bdd ->query($reqTx);
                $rsltTx = $rqTx->fetch();
                // var_dump($rsltTx);
                $sDon += $dons['somme_don']*$rsltTx['taux_taux'];
            }
        ...
        <div class="col-xl-6 connectedSortable">
        	<div class="row">
					  ...
					  <div class="row no-gutters">
						<div class="col-6 col-sm-4">
						  <div class="description-block">
							...
							<span class="description-text">Echecs <span style="font-size:10px">(Credit inssufisant / Pb Reseau)</span></span>
						  </div>
						  <!-- /.description-block -->
						</div>
						<!-- /.col -->
						...
						<div class="col-6 col-sm-4">
						  <div class="description-block">
                   			<h5 class="description-header text-green mb-5"><?php echo $nb_tranOk;?></h5>
							<span class="description-text">Succes</span>
						  </div>
						  <!-- /.description-block -->
						</div>
						<!-- /.col -->
				...
				<div class="col-xl-6">
				  ...
				  <div class="box">
					<div class="box-body">
					  <h5 class="text-center">Statistiques Dons</h5>
					  <div class="text-center">
					  	<span class="font-size-40"><?php echo $sDon;?> Fcfa</span>
					  </div>
						<div class="progress mb-5 progress-sm">
