...
if ($connect == "1") // Si le visiteur s'est identifié.
{
    ...
    WHERE artiste.id_artiste =\''.$artistID.'\' AND album.id_artiste =\''.$artistID.'\'' ;

	$alb  = $bdd ->query($req5);
	$nb_albu = $alb -> rowCount();


	$req9='SELECT * FROM son INNER JOIN artiste ON artiste.id_artiste = son.id_artiste WHERE son.id_artiste = \''.$artistID.'\' AND artiste.id_artiste =\''.$artistID.'\'' ;

	$sn  = $bdd ->query($req9);
	$nb_son = $sn -> rowCount();


// 	$req6='SELECT * FROM telechargement INNER JOIN album INNER JOIN artiste ON artiste.id_artiste = album.id_artiste WHERE album.id_album = telechargement.id_album AND artiste.id_artiste = \''.$artistID.'\' GROUP BY telechargement.id_telechargement';

// 	$te  = $bdd ->query($req6);
// 	$nb_telec = $te -> rowCount();

  
//   $req2='SELECT * FROM telechargements INNER JOIN son INNER JOIN artiste ON artiste.id_artiste = son.id_artiste WHERE son.id_son = telechargements.id_son AND artiste.id_artiste = \''.$artistID.'\' GROUP BY telechargements.id_telechargements';

//   $te  = $bdd ->query($req2);
//   $nb_telec2 = $te -> rowCount();

	$req7='SELECT *, SUBSTRING_INDEX(libelle_transaction, "-", 1) AS typeOfItem, SUBSTRING_INDEX(SUBSTRING_INDEX(libelle_transaction, "-", 2), "-", -1) AS titreOfSong, SUBSTRING_INDEX(SUBSTRING_INDEX(libelle_transaction, "-", 3), "-", -1) AS itemId  FROM transaction 
    ...
    AND 
      SUBSTRING_INDEX(SUBSTRING_INDEX(libelle_transaction, "-", 2), "-", -1) = son.tire_son
    AND 
      artiste.id_artiste = \''.$artistID.'\'
    AND 
