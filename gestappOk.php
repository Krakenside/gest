         ...
         $re .= " ORDER BY son.id_son DESC";
			...
			while ($sons = $son->fetch() ){
				...
				if ($i < 4) {
					...
					$affich_el3[$i]['lyrics'] = utf8_decode($sons['texte_lyric']);
					...
					$affich_el2[$i]['lyrics'] = utf8_decode($sons['texte_lyric']);
    ...
    if (end($lib) !== "don") {
  		if($lib[0] == 'son'){
  			...
  			$sons = $son->fetch();
				...
				$re2 = "SELECT * FROM soutCommande WHERE libelle_soutCommande ='".$sons['id_son']."-son'";
				$reqspre = $bdd -> query($re2);
				$reqsprec = $reqspre->fetch();
