...
if (isset($_GET['fct']) && ($_GET['fct'] == "1")) {
    ...
    for ($i = 0; $i < $_POST["nbr"]; $i++) {
        ...
        $reqex = "select * from son where son.titre_son =:titre and son.id_album =:id_alb  and son.id_artiste = :id_artiste ";

        $st = $bdd->prepare($reqex);

        $st->execute(array('titre' => $titre,'id_alb'=>$id_album, 'id_artiste' => $id_art));
