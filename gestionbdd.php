		...
		if (isset($_FILES[$ch_image]) AND $_FILES[$ch_image]['error'] == 0)
			...
			for ($o=0; $o < $indxfich; $o++) {
	 		 ...
	 		 $cover = (preg_match("#cover#", $ch_fich[$o])) ? 'cover-' : '';
				...
				if($_SESSION["userCompte"] == 'ADMINAFP20'){
				...
				// 	echo '<br> cest bon';
					if($tabl == 'album'){
					    	$nm = $_POST["titre_album"];
			 	$nbr_sons  = $_POST["nbr_sons"];
				 	$prix_son = $_POST["prix_album"]/$nbr_sons ;
				if($prix_son < 300){
						$prix_son = 200;
					}
				else if($prix_son<500){
						$prix_son = 300;
					}
					else{
			 		$prix_son = 500;
				 	}
				 	header("location: ajalb.php?nm=".$nm."&nbr=".$nbr_sons."&pr=".$prix_son."");
				 	}
				 else
				...
				// 			//echo '<br> cest bon';
					...
					if($tabl == 'playlist' OR $tabl == 'userplaylist'){
						...
						header("location: ../ad/ajoutusr.php?t=".$tabl."&m=1");
					} else {
					header("location: ../ad/ajout.php?t=".$tabl."&m=1");
							//echo '<br> cest bon';
					}
				}
	}
					...
					$indc++;
						...
						if($_SESSION["userCompte"] == 'ADMINAFP20'){
							...
							if($tabl == "quizz"){
								...
								$qz = $bdd -> query($re);
								$qzz = $qz->fetch();
								header("location: ../ad/liste.php?dest=quizz&s=".$qzz['id_lecon']);
							} else {
								header("location: ../ad/modifierSimpl.php?t=".$tabl."&indx=".$idCle);
							}
						}
...
function majun(){
	...
	while($champ = $req->fetch(PDO::FETCH_ASSOC))
	 {	//if(!preg_match("#id#", $champ["Field"])){
		 ...
		 if (isset($_FILES[$champ["Field"]]) AND $_FILES[$champ["Field"]]['name']!=='') {
			 $table[$i] = $champ["Field"];
  		 //echo $_POST[$champ["Field"]];
  		 $i++;
		 }
					...
					if(isset($ch_image) AND $_POST[$ch_image] !== ''){
										...
										if (isset($_FILES[$ch_image]) AND $_FILES[$ch_image]['error'] == 0)
											...
											// On peut valider le fichier et le stockerdéfinitivement
												...
												move_uploaded_file($_FILES[$ch_image]['tmp_name'], 'site/assets/images/'.$amod[$ch_image]);
